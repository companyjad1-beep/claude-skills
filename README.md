...
킥오프가 끝난 프로젝트에서 `/marketing` 한 번으로:

1. **재료 선독** — PRD·BRAND·결정 기록을 먼저 읽고 확인 요약으로 시작. 이미 결정된 타겟·문제·보이스는 다시 묻지 않음
2. **마케팅 딥 인터뷰** — 포지셔닝·메시지·채널(+명시적 제외 채널)·출시 플랜·예산 5트랙. 채널·경쟁 서비스 사실 관계는 직접 조사해 통보, 판단만 질문. 결정은 즉시 `docs/decisions/`에 기록되어 중단해도 재개 가능
3. **docs/MARKETING.md 생성** — 엘리베이터 피치, 채널별 첫 액션, 프리런칭~포스트런칭 타임라인, 측정 방법이 붙은 KPI까지. 인터뷰·조사로 추적 안 되는 수치는 안 넣음
